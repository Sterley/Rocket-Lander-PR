    ...
    for index_x, x in enumerate(np.linspace(state_min[4], state_max[4], num=definition)):
        for index_y, y in enumerate(np.linspace(state_min[3], state_max[3], num=definition)):
            ...
            for _ in range(2, 7):
                ...
                obs = np.append(obs, z)
            obs = np.append(obs, x)
            for _ in range(8, len(state_min)):
                z = random.random() - 0.5
                obs = np.append(obs, z)
...
def plot_traj_rew(model, env, deterministic, plot=True, figname="nd_actor.pdf", save_figure=True) -> None:
    ...
    for index_x, x in enumerate(np.linspace(state_min[4], state_max[4], num=definition)):
        for index_y, y in enumerate(np.linspace(state_min[3], state_max[3], num=definition)):
            ...
            portrait[definition - (1 + index_y), index_x] = rewards
    plt.figure(figsize=(10, 10))
    plt.imshow(portrait, cmap="inferno", extent=[state_min[0], state_max[0], state_min[1], state_max[1]], aspect="auto")
    ...
    final_show(save_figure, plot, figname, x_label, y_label, "Superposition de la trajectoire à la reward", "/plots/")
...
def state_reward(model, env, plot=True, figname="nd_actor.pdf", save_figure=True) -> None:
    ...
    for index_x, x in enumerate(np.linspace(state_min[0], state_max[0], num=definition)):
        for index_y, y in enumerate(np.linspace(state_min[0], state_max[0], num=definition)):
            ...
            for i in range(10):
                ...
                while True:
                    ...
                    if dones:
                        env.close()
                        break
                total_mean_rew += mean_reward
           
            portrait[definition - (1 + index_y), index_x] = total_mean_rew/10

    plt.figure(figsize=(10, 10))
    plt.imshow(portrait, cmap="inferno", extent=[state_min[0], state_max[0], state_min[1], state_max[1]], aspect="auto")
    plt.colorbar(label="Rewards")
    plt.scatter([0], [0])
    x_label, y_label = getattr(env.observation_space, "names", ["X", "Angle"])
    final_show(save_figure, plot, figname, x_label, y_label, "Actor phase portrait", "/plots/")


def plot_traj_vel_rew(model, env, deterministic, plot=True, figname="nd_actor.pdf", save_figure=True) -> None:
    ...
    state_min = env.observation_space.low
    state_max = env.observation_space.high
    for index_x, x in enumerate(np.linspace(state_min[3], state_max[3], num=definition)):
        for index_y, y in enumerate(np.linspace(state_min[3], state_max[3], num=definition)):
            env.reset()
            obs = np.array([[]])
            for _ in range(0, 1):
            ...
            for _ in range(2, 8):
                ...
                obs = np.append(obs, z)
            obs = np.append(obs, x)
            for _ in range(9, len(state_min)):
                z = random.random() - 0.5
                obs = np.append(obs, z)
            action, _states = model.predict(obs)
            obs, rewards, dones, info = env.step(action)
            portrait[index_y, index_x] = rewards
    plt.figure(figsize=(10, 10))
    plt.imshow(portrait, cmap="inferno", extent=[state_min[0], state_max[0], state_min[1], state_max[1]], aspect="auto")
    plt.colorbar(label="Rewards")
    # Add a point at the center
    plt.scatter([0], [0])
    x_label, y_label = getattr(env.observation_space, "names", ["Velocity Lineaire", "Y"])
    final_show(save_figure, plot, figname, x_label, y_label, "Superposition de la trajectoire à la reward", "/plots/")