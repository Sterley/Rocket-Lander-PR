...
from ..stable_baselines3.common.utils import obs_as_tensor as obsat
        ...
        for index_y, y in enumerate(np.linspace(y_min, y_max, num=definition)):
            ...
            with th.no_grad():
                if hasattr(model, "critic"):
                    ...
                    # For A2C/PPO/DDPG
                    action = model.forward(obsat(obs, model.device))
                    value = model.predict_values(obsat(obs, model.device), action)
            ...
            for _ in range(2, len(state_min)):
                ...
                with th.no_grad():
                    if hasattr(model, "critic"):
                        # For REINFORCE
                        if isinstance(model, REINFORCE):
                            value = model.critic.forward(obsat(obs, model.device))
                        elif isinstance(model, DDPG):
                            action = model.forward(obsat([0, 0], model.device))
                            value = model.critic.forward(obsat(obs, model.device), action)
                        else:
                            print("visu critic: algo not covered")
                    else:
                        # For A2C/PPO/DDPG
