...
def plot_nd_policy_part2(portrait, state_min, state_max, save_figure, plot, figname, env, actionname):
    plt.figure(figsize=(10, 10))
    plt.imshow(portrait, cmap="inferno", extent=[state_min[0], state_max[0], state_min[1], state_max[1]], aspect="auto")
    plt.colorbar(label=actionname)
    # Add a point at the center
    plt.scatter([0], [0])
    x_label, y_label = getattr(env.observation_space, "names", ["x", "y"])
    final_show(save_figure, plot, figname, x_label, y_label, "Rocket phase portrait : "+actionname, "/plots/")
...
def plot_nd_policy(policy, env, deterministic, plot=True, figname="nd_actor.pdf", save_figure=True) -> None:
    ...
    if env.observation_space.shape[0] <= 2:
        raise (ValueError("Observation space dimension {}, should be > 2".format(env.observation_space.shape[0])))
    definition = 100
    portrait_gimbal = np.zeros((definition, definition))
    portrait_throttle = np.zeros((definition, definition))
    portrait_force_dir = np.zeros((definition, definition))
    ...
    for index_x, x in enumerate(np.linspace(state_min[0], state_max[0], num=definition)):
        for index_y, y in enumerate(np.linspace(state_min[1], state_max[1], num=definition)):
            ...
            for _ in range(2, len(state_min)):
                ...
                obs = np.append(obs, z)
            action, _ = policy.predict(obs, deterministic=deterministic)
            portrait_gimbal[definition - (1 + index_y), index_x] = action[0]
            portrait_throttle[definition - (1 + index_y), index_x] = action[1]
            portrait_force_dir[definition - (1 + index_y), index_x] = action[2]

    plot_nd_policy_part2(portrait_gimbal, state_min, state_max, save_figure, plot, "gimbal.pdf", env, "gimbal")
    plot_nd_policy_part2(portrait_throttle, state_min, state_max, save_figure, plot, "throttle.pdf", env, "throttle")
    plot_nd_policy_part2(portrait_force_dir, state_min, state_max, save_figure, plot, "force_dir.pdf", env, "force_dir")
