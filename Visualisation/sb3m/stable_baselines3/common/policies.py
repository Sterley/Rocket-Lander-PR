...
from ...stable_baselines3.common import utils # get_device, is_vectorized_observation, obs_as_tensor
...
class BaseModel(nn.Module, ABC):
    ...
    def obs_to_tensor(self, observation: Union[np.ndarray, Dict[str, np.ndarray]]) -> Tuple[th.Tensor, bool]:
        ...
        if isinstance(observation, dict):
            ...
            for key, obs in observation.items():
                ...
                if is_image_space(obs_space):
                    ...
                    obs_ = np.array(obs)
                vectorized_env = vectorized_env or utils.is_vectorized_observation(obs_, obs_space)
        ...
        if not isinstance(observation, dict):
            ...
            vectorized_env = utils.is_vectorized_observation(observation, self.observation_space)
            # Add batch dimension if needed
            observation = observation.reshape((-1,) + self.observation_space.shape)

        observation = utils.obs_as_tensor(observation, self.device)
