...
from ...stable_baselines3.common import callbacks as clb
...
from ...stable_baselines3.common import callbacks as cbl
Schedule = Callable[[float], float]
MaybeCallback = Union[None, Callable, List[cbl.BaseCallback], cbl.BaseCallback]
...
class BaseAlgorithm(ABC):
    ...
    def _setup_lr_schedule(self) -> None:
        ...
        self.lr_schedule = utils.get_schedule_fn(self.learning_rate)
    ...
    def _init_callback(
        ...
        if isinstance(callback, list):
            callback = clb.CallbackList(callback)

        # Convert functional callback to object
        if not isinstance(callback, clb.BaseCallback):
            callback = clb.ConvertCallback(callback)

        # Create eval callback in charge of the evaluation
        if eval_env is not None:
            eval_callback = clb.EvalCallback(
                eval_env,
            ...
            callback = clb.CallbackList([callback, eval_callback])
    ...
    def set_env(self, env: GymEnv, force_reset: bool = True) -> None:
        ...
        utils.check_for_correct_spaces(env, self.observation_space, self.action_space)
    ...
    def set_random_seed(self, seed: Optional[int] = None) -> None:
        ...
        if seed is None:
            return
        utils.set_random_seed(seed, using_cuda=self.device.type == th.device("cuda").type)
    ...
    def load(
        ...
        **kwargs,
        ...
        if env is not None:
            ...
            utils.check_for_correct_spaces(env, data["observation_space"], data["action_space"])
            # Discard `_last_obs`, this will force the env to reset before training
            # See issue https://github.com/DLR-RM/stable-baselines3/issues/597
